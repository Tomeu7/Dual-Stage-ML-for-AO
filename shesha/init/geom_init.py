...
def geom_init(p_geom: conf.Param_geom, p_tel: conf.Param_tel, padding=2):
    ...
    # Useful pupil
    if p_geom.pupil_path is not None:
        # by Tomeu
        p_geom._spupil = np.load(p_geom.pupil_path).astype(np.float32)
    else:
    ...
    print("shapes, spupil {} mpupil {} ipupil {}".format(p_geom._spupil.shape, p_geom._mpupil.shape, p_geom._ipupil.shape))
