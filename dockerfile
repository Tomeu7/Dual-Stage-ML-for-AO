...
RUN conda create -n dual-stage-ml-for-ao python=3.8.13 -y

# Activate the environment by specifying it in the ENTRYPOINT
ENTRYPOINT ["conda", "run", "-n", "dual-stage-ml-for-ao", "/bin/bash", "-c"]

# Other packages
RUN conda run -n dual-stage-ml-for-ao conda install -c compass compass==5.2.1 -y
# Printing if compass is installed
RUN conda run -n dual-stage-ml-for-ao conda list compass
...
RUN conda run conda activate dual-stage-ml-for-ao

# Ensures that the environment is activated
ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "dual-stage-ml-for-ao", "/bin/bash", "-c"]